...
# Streamlit Cloud runs this script from its own directory (app/ui/), not the
# repo root, so the `app` package (app.database, app.agent) isn't importable
# by default the way it is when running `streamlit run app/ui/streamlit_app.py`
# from the repo root locally. Add the repo root explicitly.
            ...
            try:
                ...
                if raw:
                    with st.expander("Show technical details"):
                        st.markdown(f"<div class='sql-block'>{raw}</div>", unsafe_allow_html=True)
